 ...
 * Shows only active ZKP positions with Private tags
 */
...
export const ActivePositions: React.FC<ActivePositionsProps> = ({
  ...
  // Pagination logic for active positions only
  const { paginatedTrades, totalPages, hasNextPage, hasPrevPage } = useMemo(() => {
    const sortedTrades = [...activePositions].sort((a, b) => b.timestamp - a.timestamp);
    const startIndex = (currentPage - 1) * tradesPerPage;
    const endIndex = startIndex + tradesPerPage;
    const paginatedTrades = sortedTrades.slice(startIndex, endIndex);
    const totalPages = Math.ceil(activePositions.length / tradesPerPage);
  ...
  }, [activePositions, currentPage, tradesPerPage]);
  ...
  const handleClosePosition = async (tradeId: string) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.info('💰 Collateral returned to DarkPool balance');
  ...
  const calculatePnL = (trade: TradeHistoryItem) => {
    // Simple PnL calculation (in real app, this would use current market price)
    const pnlValue = Math.random() * 200 - 100; // Mock PnL between -100 and +100
    const pnlPercent = (pnlValue / parseFloat(trade.collateral.replace(/[^\d.-]/g, ''))) * 100;
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle className="text-xl font-bold text-neon-green flex items-center space-x-2">
            ...
            <span>Active ZKP Positions</span>
      ...
      <CardContent className="space-y-4">
        {activePositions.length === 0 ? (
          <div className="text-center py-8 text-gray-400">
            ...
            <p className="text-sm">Execute a ZKP trade to create your first position</p>
          </div>
        ) : (
          <>
                    ...
                    <CardContent className="p-4">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-4">
                        ...
                        <div>
                          ...
                          <div className={`text-sm font-medium ${pnl.value >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                            {pnl.value >= 0 ? '+' : ''}{pnl.value.toFixed(2)} USDC
                            <div className="text-xs opacity-75">({pnl.percent >= 0 ? '+' : ''}{pnl.percent.toFixed(2)}%)</div>
                          </div>
                        </div>
                        <div>
                          ...
                          <div className="bg-black/40 rounded-lg p-3">
                            ...
                            <div className="grid grid-cols-2 gap-3 text-xs">
                              ...
                              <div>
                                ...
                                <Badge className="ml-2 h-4 bg-blue-500/20 text-blue-300 text-xs">
                                  Active
                                </Badge>
                              </div>
                            </div>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between pt-4 border-t border-neon-purple/20">
                <div className="text-sm text-gray-400">
                  Page {currentPage} of {totalPages} ({activePositions.length} active positions)
                </div>
                <div className="flex items-center space-x-2">
                  <Button
